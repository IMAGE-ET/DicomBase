...
////
//// File: CollectionSynchronizeHelper.cs
////
//// Summary:
////
////
        ...
        where TDestItem : class
    {
        ...
        public void Synchronize(ICollection<TDestItem> dest, ICollection<TSourceItem> source)
        {
            ...
            foreach (TSourceItem sourceItem in source)
            {
                ...
                TSourceItem item = sourceItem;
                TDestItem existingItem = existing.FirstOrDefault(destItem => CompareItems(destItem, item));
            ...
            if (unProcessed.Count > 0)
            {
                if (_allowRemove)
                {
                    foreach (TDestItem destItem in unProcessed)
                    {
