...
namespace UIH.RT.TMS.Dicom.Iod.Modules.Tests
{
	...
	public class OverlayPlaneTests
	{
		...
		private static void SetImage(IDicomElementProvider dataset, byte[] pixelData, int rows, int columns, int frames, int bitsAllocated, int bitsStored, int highBit, bool isSigned)
		{
			DicomOverlayTestHelper.SetImagePixels(dataset, pixelData, rows, columns, frames, bitsAllocated, bitsStored, highBit, isSigned);
		}

		private static void SetOverlay(IDicomElementProvider dataset, int overlayIndex, bool[] overlayData, OverlayType type, Point origin, int rows, int columns, bool bigEndian, bool useOW)
		{
			DicomOverlayTestHelper.AddOverlayPlane(dataset, overlayIndex, overlayData, type, origin, rows, columns, bigEndian, useOW);
		}

		private static void SetOverlay(IDicomElementProvider dataset, int overlayIndex, bool[] overlayData, OverlayType type, Point origin, int rows, int columns, int? frames, bool bigEndian, bool useOW)
		{
			DicomOverlayTestHelper.AddOverlayPlane(dataset, overlayIndex, overlayData, type, origin, rows, columns, frames, bigEndian, useOW);
		}

		private static void SetOverlay(IDicomElementProvider dataset, int overlayIndex, bool[] overlayData, OverlayType type, Point origin, int rows, int columns, int? frames, int? frameOrigin, bool bigEndian, bool useOW)
		{
		...
		private static void SetOverlay(IDicomElementProvider dataset, int overlayIndex, bool[] overlayData, OverlayType type, Point origin, int bitPosition, bool bigEndian)
		{
