...
{
    /// <summary>
    /// The DicomDataset class models an a collection of DICOM attributes.
    /// </summary>
    /// <remarks>
    /// <para>
    /// This class represents a collection of <see cref="DicomElement"/> classes.  It is used by the <see cref="DicomMessageBase"/> class to 
    ...
    /// </para>
    /// </remarks>
	public class DicomDataset : IEnumerable<DicomElement>, IDicomElementProvider
    {
        #region Member Variables
        private readonly SortedDictionary<uint, DicomElement> _attributeList = new SortedDictionary<uint, DicomElement>();
        ...
        /// <see cref="DicomDataset"/> instances.</para>
        /// </remarks>
        ...
        public DicomDataset(uint startTag, uint endTag)
        {
            _startTag = startTag;
            _endTag = endTag;
        }
		...
		/// <summary>
		/// Internal constructor used when creating a copy of an DicomDataset.
		/// </summary>
		...
		internal DicomDataset(DicomDataset source, bool copyBinary, bool copyPrivate, bool copyUnknown)
			: this(source, copyBinary, copyPrivate, copyUnknown, 0xFFFFFFFF)
        {

        }

        internal DicomDataset(DicomDataset source, bool copyBinary, bool copyPrivate, bool copyUnknown, uint stopTag)
        {
        	_startTag = source.StartTagValue;
        	_endTag = source.EndTagValue;
        	_specificCharacterSet = source.SpecificCharacterSet;

            foreach (DicomElement attrib in source)
            {
				...
				if (!copyUnknown && attrib is DicomElementUn)
					continue;

                if (copyBinary ||
                      (!(attrib is DicomElementOb)
                    && !(attrib is DicomElementOw)
                    && !(attrib is DicomElementOf)
        ...
        public bool IsEmpty()
        {
            foreach (DicomElement attr in this)
            {
                if (attr.Count > 0)
                    return false;
            }
        ...
        /// <summary>
        /// Check if a tag is contained in an DicomDataset and has a value.
        /// </summary>
		...
		/// <param name="element">The output attribute.  Null if the attribute doesn't exist in the collection.  Will be set if the attribute exists, but is empty.</param>
		/// <returns>true if the attribute exists and is not empty.</returns>
		public bool TryGetAttribute(uint tag, out DicomElement element)
		{
			if (!_attributeList.TryGetValue(tag, out element))
			{
				return false;
			}
		...
		/// <param name="element">The output attribute.  Null if the attribute doesn't exist in the collection.  Will be set if the attribute exists, but is empty.</param>
		/// <returns>true if the attribute exists and is not empty.</returns>
		public bool TryGetAttribute(DicomTag tag, out DicomElement element)
		{
			if (!_attributeList.TryGetValue(tag.TagValue, out element))
			{
				return false;
			}
        ...
        /// <summary>
        /// Get a <see cref="DicomElement"/> for a specific DICOM tag.
        /// </summary>
        /// <remarks>
        /// <para>If <paramref name="tag"/> does not exist in the collection, a new <see cref="DicomElement"/>
        ...
        /// <returns>A <see cref="DicomElement"/> instance.</returns>
        public DicomElement GetElement(uint tag)
        {
            DicomElement attr;
        ...
        /// <param name="tag">The DICOM tag.</param>
        /// <returns>A <see cref="DicomElement"/> instance.</returns>
        public DicomElement GetElement(DicomTag tag)
        {
            DicomElement attr;
        ...
        public bool RemoveElement(uint tag)
        {
            DicomElement attr;
            if (!_attributeList.TryGetValue(tag, out attr))
            {
        ...
        /// <returns><b>true</b> if the tag is successfully removed. The method also return true if the specified tag does not exist in the collection.</returns>
        public bool RemoveElement(DicomTag tag)
        {
            return RemoveElement(tag.TagValue);
        }
        ...
        /// <para>If the tag does not exist within the collection, a new <see cref="DicomElement"/>
            ...
            set 
            {
                ...
                else
                {
					if (value.Tag.TagValue != tag)
						throw new DicomException("Tag being set does not match tag in DicomElement");
        ...
        /// <summary>
        /// Create a duplicate copy of the DicomDataset.
        /// </summary>
        /// <remarks>
        /// <para>
        /// This method creates a copy of all of the attributes within the DicomDataset and returns 
        /// a new copy.  Note that binary attributes with a VR of OB, OW, OF, and UN are copied.
        /// </para>
        /// </remarks>
        /// <returns>A new DicomDataset.</returns>
        public virtual DicomDataset Copy()
        {
        ...
        /// Create a duplicate copy of the DicomDataset.
        /// </summary>
        /// <remarks>This method will not copy <see cref="DicomElementOb"/>,
        /// <see cref="DicomElementOw"/> and <see cref="DicomElementOf"/>
		...
		/// <returns>a new DicomDataset.</returns>
        public virtual DicomDataset Copy(bool copyBinary, bool copyPrivate, bool copyUnknown)
        {
			return new DicomDataset(this, copyBinary, copyPrivate, copyUnknown);
        }
		...
		/// <param name="copyUnknown">Flag to set if UN VR attributes will be copied.</param>
		/// <returns>a new DicomDataset.</returns>
		public virtual DicomDataset Copy(bool copyBinary, bool copyPrivate, bool copyUnknown, uint stopTag)
		{
			return new DicomDataset(this, copyBinary, copyPrivate, copyUnknown, stopTag);
		}
		...
		{
		    List<DicomElementComparisonResult> failureReason = new List<DicomElementComparisonResult>();
        ...
        /// <returns>true if the collections are equal.</returns>
        public bool Equals(object obj, ref List<DicomElementComparisonResult> failures)
        {
            DicomDataset a = obj as DicomDataset;
            if (a == null)
            {
                DicomElementComparisonResult result = new DicomElementComparisonResult();
            ...
            IEnumerator<DicomElement> thisEnumerator = GetEnumerator();
            IEnumerator<DicomElement> compareEnumerator = a.GetEnumerator();
            ...
            for (; ; )
            {
                ...
                DicomElement thisAttrib = thisEnumerator.Current;
                DicomElement compareAttrib = compareEnumerator.Current;
        ...
        {
            uint length = 0;
            foreach (DicomElement item in this)
            {
        ...
        private static object LoadDicomFieldValue(DicomElement elem, Type vtype, DicomFieldDefault deflt, bool udzl)
        {
            if (vtype.IsSubclassOf(typeof(DicomElement)))
            {
        ...
        private void SaveDicomFieldValue(DicomTag tag, object value, bool createEmpty, bool setNullIfEmpty)
        {
            if (value != null && value != DBNull.Value)
            {
                ...
                {
                    this[tag] = (DicomElement)value;
                }
                else if (vtype.IsSubclassOf(typeof(DicomSequenceItem)))
                {
                    DicomElement elem = this[tag];
