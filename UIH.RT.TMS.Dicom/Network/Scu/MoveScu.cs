...
namespace UIH.RT.TMS.Dicom.Network.Scu
{
    ...
    public abstract class MoveScuBase : ScuBase 
    {
    	...
    	public QueryRetrieveLevel CurrentLevel
    	{
    		get
    		{
				return IodBase.ParseEnum(_dicomDataset[DicomTags.QueryRetrieveLevel].GetString(0, String.Empty), QueryRetrieveLevel.None);
    		}
    	}
		...
		public virtual void AddStudyInstanceUid(string studyInstanceUid)
        {
            ...
            _dicomDataset[DicomTags.StudyInstanceUid].AppendString(studyInstanceUid);
        }
        ...
        public virtual void AddPatientId(string patientId)
        {
            ...
            _dicomDataset[DicomTags.PatientId].AppendString(patientId);
        }
        ...
        {
            if (_dicomDataset[DicomTags.StudyInstanceUid].Count != 1)
				throw new InvalidOperationException("Exactly one study uid must be specified.");

			CheckForOtherLevel(QueryRetrieveLevel.Series);
            _dicomDataset[DicomTags.SeriesInstanceUid].AppendString(seriesInstanceUid);
        }
        ...
        private void SendMoveRequest(DicomClient client, ClientAssociationParameters association)
        {
            ...
            foreach (DicomElement dicomAttribute in _dicomDataset)
            {
                // Need to do it this way in case the attribute is blank
                DicomElement dicomAttribute2 = dicomMessage.DataSet[dicomAttribute.Tag];
        ...
        {
            QueryRetrieveLevel currentLevel = IodBase.ParseEnum(_dicomDataset[DicomTags.QueryRetrieveLevel].GetString(0, String.Empty), QueryRetrieveLevel.None);

			// Use the highest (most granular) query level (Image > Series > Study > Patient)
			if (queryRetrieveLevel > currentLevel)
			{
				IodBase.SetAttributeFromEnum(_dicomDataset[DicomTags.QueryRetrieveLevel], queryRetrieveLevel);
			}
		...
		public override void OnReceiveResponseMessage(DicomClient client, ClientAssociationParameters association, byte presentationID, DicomMessage message)
        {
        	...
        	else
        	{
				...
				if (message.Status.Status != DicomState.Success)
				{
					...
					{
						if (LogInformation) LogAdapter.Logger.Info("Client cancelled Move Scu operation.");
					}
				}
				else
				{
					if (LogInformation) LogAdapter.Logger.Info("Success status received in Move Scu!");
				}
		...
		public override void OnDimseTimeout(DicomClient client, ClientAssociationParameters association)
		{
			...
			FailureDescription =
				String.Format("Timeout Expired ({0} seconds) for remote host {1} when processing C-MOVE-RQ, aborting connection", association.ReadTimeout/1000,
				              RemoteAE);
			LogAdapter.Logger.Error(FailureDescription);
    ...
    public class PatientRootMoveScu : MoveScuBase
    {
		...
		{
			if (base.DicomDataset[DicomTags.PatientId].Count != 1)
