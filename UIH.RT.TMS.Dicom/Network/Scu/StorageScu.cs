	...
	public class StorageScu : ScuBase
	{
		    ...
		    ThreadPool.QueueUserWorkItem(delegate
		                                     {
                                                 try
                                                 {
                                                     ...
                                                     while (ok == false)
                                                     {
                                                         ...
                                                         {
                                                             LogAdapter.Logger.Info("Completed sending C-STORE-RQ messages, releasing association.");
                                                 ...
                                                 {
                                                     LogAdapter.Logger.Error("Error when sending C-STORE-RQ messages, aborted on-going send operations");
                                                     try
                                                     {
                                                     ...
                                                     {
                                                         LogAdapter.Logger.Error("Error attempting to abort association");
                                                     }                                                     
                                                 }
		                                     }, null);
		}
        ...
        private void LogError(StorageInstance instance, DicomMessage msg, DicomStatus dicomStatus)
        {
            if (dicomStatus == DicomStatuses.SOPClassNotSupported)
            {
                ...
                LogAdapter.Logger.Error(log.ToString());
            }
        }
