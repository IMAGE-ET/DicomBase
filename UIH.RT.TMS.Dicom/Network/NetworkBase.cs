...
namespace UIH.RT.TMS.Dicom.Network
{
    ...
    public abstract class NetworkBase
    {
        ...
        {
            LogAdapter.Logger.Error("Unexpected User exception, description: " + description);
            switch (State)
            {
        ...
        public delegate void DimseMessageSendingEventHandler(
            AssociationParameters assoc, byte presentationContextID, DicomDataset command,
            DicomDataset dataset);
        ...
        public void SendAssociateAccept(AssociationParameters associate)
        {
            ...
            {
                LogAdapter.Logger.Error("Error attempting to send association accept at invalid time in association.");
                SendAssociateAbort(DicomAbortSource.ServiceProvider, DicomAbortReason.NotSpecified);
        ...
        public void SendAssociateReject(DicomRejectResult result, DicomRejectSource source, DicomRejectReason reason)
        {
            if (State != DicomAssociationState.Sta3_AwaitingLocalAAssociationResponsePrimative)
            {
                LogAdapter.Logger.Error("Error attempting to send association reject at invalid time in association.");
                SendAssociateAbort(DicomAbortSource.ServiceProvider, DicomAbortReason.NotSpecified);
                throw new DicomNetworkException(
        ...
        public void SendReleaseRequest()
        {
            ...
            {
                LogAdapter.Logger.Error("Unexpected attempt to send Release Request when in invalid state.");
                return;
            }
                    ...
                    else if (DateTime.Now > GetDimseTimeout())
                    {
                    	...
                    	switch (State)
                        {
                        	...
                        	case DicomAssociationState.Sta2_TransportConnectionOpen:
                        		...
                        		LogAdapter.Logger.Error(errorMessage);
                        		State = DicomAssociationState.Sta13_AwaitingTransportConnectionClose;
        ...
        private bool ProcessRawPDU(RawPDU raw)
        {
            try
            {
                switch (raw.Type)
                {
                    case 0x01:
                        {
                            ...
                            if (State != DicomAssociationState.Sta13_AwaitingTransportConnectionClose &&
                                State != DicomAssociationState.Sta6_AssociationEstablished)
                            {
                                LogAdapter.Logger.Error("Association incorrectly not accepted or rejected, aborting.");
                                return false;
                            }
                    ...
                    case 0xFF:
                        {
                            LogAdapter.Logger.Error("Unexpected PDU type: 0xFF.  Potential parsing error.");
                            return false;
                        }
                    default:
        	...
        	try
            {
                ...
                foreach (PDV pdv in pdu.PDVs)
                {
                    ...
                    if (pdv.IsCommand)
                    {
                        ...
                        {
                            _dimse.Command = new DicomDataset(0x00000000, 0x0000FFFF);
                        }
                        ...
                        {
							LogAdapter.Logger.Error("Unexpected parsing error when reading command group elements.");
                            return false;
                        }
