...
namespace UIH.RT.TMS.Dicom
{
    ...
    public class DicomFile : DicomMessageBase, IComparable<DicomFile>
    {
        ...
        /// <param name="metaInfo">A <see cref="DicomDataset"/> for the MetaInfo (group 0x0002 attributes).</param>
        /// <param name="dataSet">A <see cref="DicomDataset"/> for the DataSet.</param>
        public DicomFile(String filename, DicomDataset metaInfo, DicomDataset dataSet)
        {
            MetaInfo = metaInfo;
            DataSet = dataSet;
            _filename = filename;
            ...
            //set media storage sop class uid(0002,0002), the value should be same with tag sop class uid (0008,0016)
            MetaInfo[DicomTags.MediaStorageSopClassUid].SetStringValue(DataSet[DicomTags.SopClassUid]);

            //set media storage sop instance uid(0002,0003), the value should be same with tag Sop Instance Uid (0008,0018)
            MetaInfo[DicomTags.MediaStorageSopInstanceUid].SetStringValue(DataSet[DicomTags.SopInstanceUid]);
        ...
        {
            MetaInfo = new DicomDataset(0x00020000, 0x0002FFFF);
            DataSet = new DicomDataset(0x00040000, 0xFFFFFFFF);
        ...
        /// </para>
        /// <para>
        /// A new <see cref="DicomDataset"/> is created for the MetaInfo.  The 
