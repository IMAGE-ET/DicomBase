...
namespace UIH.RT.TMS.Dicom.Utilities.Anonymization
{
	...
	public partial class DicomAnonymizer
	{
		...
		/// </summary>
		/// <remarks>
		/// By default, text element values will be encoded using UTF-8 Unicode (ISO-IR 192).
		...
		private void Anonymize(DicomDataset dataset)
		{
			++_recursionLevel;
			...
			foreach (DicomElement attribute in dataset)
			{
				_currentTagPath[_recursionLevel] = attribute.Tag.TagValue;

				foreach (DicomDataset subCollection in GetSubCollections(attribute))
		...
		private void RemapUidAttribute(DicomElement element)
		{
			int i = 0;
			foreach (string oldUid in (string[]) element.Values)
			{
				if (!String.IsNullOrEmpty(oldUid))
				{
					string newUid;
					...
					element.SetString(i++, newUid);
				}
			}
		}

		private void AdjustDateTimeAttribute(DicomElement element)
		{
			if (_studyDateDifference != null)
			{
				int i = 0;
				if (element is DicomElementDa)
				{
					foreach (string date in (string[]) element.Values)
						element.SetString(i++, AdjustDate(date, _studyDateDifference.Value));
				}
				else if (element is DicomElementDt)
				{
					foreach (string datetime in (string[]) element.Values)
						element.SetString(i++, AdjustDateTime(datetime, _studyDateDifference.Value));
				}
			}
			else
			{
		...
		private static bool IsPrivateAttribute(DicomElement element)
		{
			return (element.Tag.IsPrivate || element.Tag.Name == "Private Tag");
		}
		...
		private static IEnumerable<DicomDataset> GetSubCollections(DicomElement element)
		{
			if (element is DicomElementSq && element.Values != null)
			{
				DicomSequenceItem[] sequenceItems = (DicomSequenceItem[]) element.Values;
